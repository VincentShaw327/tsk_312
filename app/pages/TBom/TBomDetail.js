...
import { Input, Modal, Radio, Table, Form, Button, Popconfirm, message, Select } from 'antd';
...
export default class TBomDetail extends Component {
    ...
    getMtrModelList() {
        ...
        TPostData(
 '/api/TWms/material_model', 'ListActive', dat,
            ( res ) => {
 ...
 '/api/TProduct/product_model', 'ListActive', dat,
    ...
    handleCreat=( data ) => {
        ...
        const {
MtrlType, MtrlModelUUID, ProModelUUID, Unit, UsedNumber,
} = data;
        const dat = {
...
this.url, 'AddItem', dat,
    ...
    handleUpdate=( data ) => {
        ...
        const {
 MtrlType, MtrlModelUUID, ProModelUUID, Unit, UsedNumber, LossRate, ConstLossNumber,
 } = data;
...
this.url, 'RemoveItem', dat,
    ...
    toggleMtrlType=( e ) => {
        ...
        this.setState( { isBaseMtrl: value === 0 } );
    }
        ...
        const columns = [
            ...
            },
            {
                ...
                <span>
                    <a
                    // href="#"
                      onKeyDown={() => this.toggleEModalShow( record )}
                      onClick={() => this.toggleEModalShow( record )}
                    >编辑
                    </a>
            ...
            <div className="cardContent">
                ...
                >
                        <Form layout="horizontal">
                            ...
                            >
                                {
                                    getFieldDecorator( 'MtrlType', { rules: [{ required: true, message: '请选择物料类型' }], initialValue: MtrlTypeValue } )( <RadioGroup onChange={this.toggleMtrlType}>
                            ...
                            >
                                {
                                    getFieldDecorator( 'MtrlModelUUID', { rules: !isBaseMtrl ? [] : [{ required: true, message: '请选择物料型号' }], initialValue: MtrlModelValue } )( <Select disabled={!isBaseMtrl}>
                                        {
                                            MtrModelList.map( ( item, i, arr ) => <Option key={i} value={item.value}>{item.text || item.value}</Option> )
                                        }
                                                                                                                                                                                </Select> )}
                            </FormItem>
                            <FormItem
                            ...
                            >
                                {
                                    getFieldDecorator( 'ProModelUUID', { rules: isBaseMtrl ? [] : [{ required: true, message: '请选择产品型号' }], initialValue: ProModelValue } )( <Select disabled={isBaseMtrl}>
                                        {
                                            ProductModelList.map( ( item, i, arr ) => <Option key={i} value={item.value}>{item.text || item.value}</Option> )
                                        }
                                                                                                                                                                             </Select> )}
                            </FormItem>
                            <FormItem
                            ...
                            >
                                {
                                    getFieldDecorator( 'UsedNumber', { rules: [{ required: true, message: '请输入使用量' }], initialValue: UsedNumberValue } )( <Input placeholder="请输入使用量" /> )
                                }
                            </FormItem>
                            <FormItem
                              label="单位"
                            ...
                            >
                                {
                                    getFieldDecorator( 'Unit', { rules: [{ required: true, message: '请输入单位' }], initialValue: UnitValue } )( <Input placeholder="请输入物料单位" /> )
                                }
                            </FormItem>
                        </Form>
                </Modal>
                ...
                >
                        <Form layout="horizontal">
                            ...
                            >
                                {
                                    getFieldDecorator( 'UsedNumber', { rules: [{ required: true, message: '使用量不能为空' }], initialValue: UsedNumberValue } )( <Input placeholder="请输入使用量" /> )
                                }
                            </FormItem>
                            <FormItem
                              label="单位"
                            ...
                            >
                                {
                                    getFieldDecorator( 'Unit', { rules: [{ required: true, message: '单位不能为空' }], initialValue: UnitValue } )( <Input placeholder="请输入物料单位" /> )
                                }
                            </FormItem>
                            <FormItem
                              label="损耗率"
                            ...
                            >
                                {
                                    getFieldDecorator( 'LossRate', { rules: [{ required: true, message: '损耗率不能为空！' }], initialValue: LossRateValue } )( <Input placeholder="请输入损耗率" /> )
                                }
                            </FormItem>
                            <FormItem
                              label="固定损耗率"
                            ...
                            >
                                {
                                    getFieldDecorator( 'ConstLossNumber', { rules: [{ required: true, message: '固定损耗不能为空！' }], initialValue: LossNumberValue } )( <Input placeholder="请输入固定损耗率" /> )
                                }
                            </FormItem>
                        </Form>
                </Modal>
