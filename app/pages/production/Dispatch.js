...
import { fetchTaskList } from 'actions/production';
...
export default class App extends Component {
    ...
    getDispatchLotList() {
        ...
        // "ListJobTask"
        TPostData(
            '/api/tmanufacture/manufacture/ListWorkOrder', 'ListWorkOrder', dat,
            ( res ) => {
                const list = [];
                ...
                render: ( e1, record ) => {
                    ...
                    if ( record.Status && record.Status === 0 ) {
                        operate = (
                            <span>
                                <a
                                  onKeyDown={() => this.toggleModalShow( 'submitModalShow', record )}
                                  onClick={() => this.toggleModalShow( 'submitModalShow', record )}
                                >报工
                                </a>
                            </span>
                        )
                    } else if ( record.Status && record.Status === 1 ) {
                        operate = (
                            /* <span>
                                <Popconfirm
                                    placement="topLeft"
                            ...
                            <span>
                                ...
                                <a
                                  onKeyDown={() => this.toggleModalShow( 'startModalShow', record )}
                                  onClick={() => this.toggleModalShow( 'startModalShow', record )}
                                >开始生产
                                </a>
                            </span>
                        )
                        ...
                        operate = (
                            <span>
                                ...
                                <a
                                  onKeyDown={() => this.toggleModalShow( 'pauseModalShow', record )}
                                  onClick={() => this.toggleModalShow( 'pauseModalShow', record )}
                                >暂停生产
                                </a>
                                <span className="ant-divider" />
                                <a
                                  onKeyDown={this.toggleModalShow( 'stopModalShow', record )}
                                  onClick={this.toggleModalShow( 'stopModalShow', record )}
                                >停止生产
                                </a>
                                <span className="ant-divider" />
                                <a
                    ...
                    <div style={{ margin: '20px 0', overflow: 'auto', zoom: 1 }}>
                        <div style={{ float: 'right' }}>
                            <Form layout="inline">
                                ...
                                <FormItem label="边框">
                                    <Switch checked={bordered} onChange={this.handleToggleBorder} />
                                </FormItem>
                                <FormItem label="大小">
